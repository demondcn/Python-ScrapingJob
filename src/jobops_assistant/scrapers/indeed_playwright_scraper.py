...
INDEED_PLAYWRIGHT_MANUAL_CHALLENGE_MESSAGE = (
    "Indeed Playwright: se detecto Security Check / captcha. "
    "Resuelvelo manualmente en la ventana abierta y presiona ENTER para reintentar."
)
...
class IndeedPlaywrightJobScraper(PlaywrightJobScraper, IndeedSeleniumJobScraper):
    ...
    def fetch_search_results(self, source) -> str:
        ...
        try:
            html, final_url, status_code = self._load_rendered_html(
                driver,
                requested_url,
            ...
            self.last_response_debug = self._build_debug_snapshot(
                requested_url,
                final_url,
                html,
                status_code,
            ...
            if self._is_security_check_page(html, final_url):
                print(INDEED_PLAYWRIGHT_MANUAL_CHALLENGE_MESSAGE)
                input()
    ...
    def _needs_manual_login(self, html: str, current_url: str) -> bool:
        ...
        return "/account/login" in current_url_normalized or any(
            signal in visible_text for signal in login_signals
        )
    ...
    def _is_security_check_page(self, html: str, current_url: str = "") -> bool:
        html_normalized = self._normalize_text(html)
        url_normalized = self._normalize_text(current_url)
        ...
        return any(signal in html_normalized for signal in security_signals) or (
            "__cf_chl" in url_normalized
        )