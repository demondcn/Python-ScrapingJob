...
COMPUTRABAJO_PLAYWRIGHT_CARD_SELECTORS = (
    ".box_offer",
    ".js-card",
    "article",
    "[data-id]",
...
COMPUTRABAJO_PLAYWRIGHT_BLOCK_SIGNALS = (
    "403",
    "access denied",
    "captcha",
    "forbidden",
)
COMPUTRABAJO_POST_NAVIGATION_WAIT_SECONDS = 2.0
...
class ComputrabajoPlaywrightJobScraper(PlaywrightJobScraper, ComputrabajoJobScraper):
    ...
    def fetch_search_results(self, source) -> str:
        ...
        try:
            ...
            if state == COMPUTRABAJO_STATE_BLOCKED or self._contains_blocked_page_text(html):
    ...
    def _wait_for_rendered_dom(self, driver, selectors: tuple[str, ...]) -> None:
        timeout = max(1, min(int(getattr(self.settings, "playwright_page_load_timeout", 1) or 1), 2))
        wait_supported = getattr(driver, "wait_for_any_selector", None)
        selector_pool = tuple(dict.fromkeys(COMPUTRABAJO_PLAYWRIGHT_CARD_SELECTORS + selectors))
        if self._has_any_selector(driver, selector_pool):
            return
        if callable(wait_supported) and wait_supported(selector_pool, timeout=timeout):
            return
        if self._has_any_selector(driver, ("body",)):
            return
        if callable(wait_supported) and wait_supported(("body",), timeout=timeout):
            return
    ...
    def _scroll_page(self, driver) -> None:
        wait_for_timeout = getattr(driver, "wait_for_timeout", None)
        scroll_count = max(3, min(int(getattr(self.settings, "playwright_max_scrolls", 4) or 4), 6))
        pause_seconds = max(1, min(int(getattr(self.settings, "playwright_scroll_pause", 1) or 1), 2))
        for _ in range(scroll_count):
            driver.execute_script("window.scrollBy(0, Math.max(500, Math.floor(window.innerHeight * 0.85)));")
            if callable(wait_for_timeout):
                wait_for_timeout(pause_seconds)

    def _settle_after_navigation(self, driver) -> None:
        timeout = max(1, min(int(getattr(self.settings, "playwright_page_load_timeout", 1) or 1), 3))
        wait_for_load_state = getattr(driver, "wait_for_load_state", None)
        if callable(wait_for_load_state):
            try:
                wait_for_load_state("domcontentloaded", timeout=timeout)
            except Exception as exc:
                self._debug_log(f"[computrabajo] domcontentloaded_wait_skipped={exc}", level="warning")
        wait_for_timeout = getattr(driver, "wait_for_timeout", None)
        if callable(wait_for_timeout):
            wait_for_timeout(COMPUTRABAJO_POST_NAVIGATION_WAIT_SECONDS)
    ...
    def _contains_blocked_page_text(self, html: str) -> bool:
        normalized_html = _normalize_computrabajo_text(html)
        return any(token in normalized_html for token in COMPUTRABAJO_PLAYWRIGHT_BLOCK_SIGNALS)
